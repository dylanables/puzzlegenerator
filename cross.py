...
class Crossword(object):
    ...
    def check_fit_score(self, col, row, vertical, word):
        '''
        Returns score: 0 means no fit, 1 means a fit, 2+ means a cross.
    ...
    def solution(self): # return solution grid
        ...
        for r in range(self.rows):
            ...
            for c in range(self.cols):
                row_copy.append({'num': 0, 'letter': self.grid[r][c]})
    ...
    def display(self, order=True): # return (and order/number wordlist) the grid minus the words adding the numbers
        ...
        for r in range(copy.rows):
            for c in range(copy.cols):
                if str(copy.get_cell(c, r)).isalpha():
                    copy.set_cell(c, r, '*')

        return copy
 
    def clues(self): # must order first
        acrossStr = 'Across: \n'
        downStr = 'Down: \n'
        for word in self.current_word_list:
...
class Word(object):
    ...
    def orientation(self): # return down or across
        if self.vertical: 
            return 'down'
        else: 
            return 'across'
...
def crossword():
    if request.method == 'GET':
        ...
        if prompt:
            ...
            a = Crossword(grid_size, grid_size, '-', 5000, words_and_clues)
            ...
            clues = a.clues().split('\n')
            print(solution)
            return render_template("crossword.html", prompt=string.capwords(prompt), grid=grid, clues=clues, solution=solution)
        else:
            return render_template("crossword.html", prompt=prompt)
    else:
        prompt = ''
        return render_template("crossword.html", prompt=prompt)